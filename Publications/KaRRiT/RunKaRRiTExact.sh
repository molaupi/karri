#!/bin/bash
# Runs KaRRiT in exact configuration.
...
sourceDir=./../..
binaryDir=$sourceDir/Build/Release/KaRRiTExact/
cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_PREFIX_PATH="${dependencyInstallDir}"\
  ...
  -DKARRI_TRANSFER_HEURISTIC_LEVEL=0 \
...
$binaryDir/Launchers/karri \
  ...
  -o $outputDir/${name}/karrit-exact_${radius}_${waitTime}_${maxNumPDLocs}