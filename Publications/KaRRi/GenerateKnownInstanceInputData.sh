...
if [ "${instanceName}" = "Berlin-1pct" ]; then
  ...
  sh GenerateInputDataByTransformingMATSimData.sh   \
    $baseDir                                        \
    Berlin                                          \
    Berlin-1pct                                     \
    ./MATSim_Output/Requests_MATSim_Berlin-1pct.csv \
    ./MATSim_Output/Vehicles_MATSim_Berlin-1pct.csv \
    ./MATSim_Output/MATSim_Berlin.gr.bin            \
    $passengerMode                                  \
    ./BoundaryPolygons/Berlin_Inner.poly            \
    $dependencyInstallDir
  exit 0
fi
...
if [ "${instanceName}" = "Berlin-10pct" ]; then
  ...
  sh GenerateInputDataByTransformingMATSimData.sh    \
    ...
    Berlin                                           \
    Berlin-10pct                                     \
    ./MATSim_Output/Requests_MATSim_Berlin-10pct.csv \
    ./MATSim_Output/Vehicles_MATSim_Berlin-10pct.csv \
    ./MATSim_Output/MATSim_Berlin.gr.bin             \
    $passengerMode                                   \
    ./BoundaryPolygons/Berlin_Inner.poly             \
    $dependencyInstallDir
...
if [ "${instanceName}" = "Ruhr-1pct" ]; then
  ...
  sh GenerateInputDataGeneric.sh                    \
    $baseDir                                        \
    Ruhr                                            \
    ./BoundaryPolygons/                             \
    Ruhr-1pct                                       \
