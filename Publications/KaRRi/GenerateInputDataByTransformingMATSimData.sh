...
innerAreaFile=$8           # Optional: boundary file for inner area of network that contains all request locations; if set, passenger network is limited to inner area
dependencyInstallDir=$9    # Optional: Path to headers of dependencies
...
$binaryDir/RawData/OsmToCarAndPassengerGraph -psg-mode ${passengerMode} -no-union-nodes -no-veh-on-service -b $innerAreaFile -a lat_lng osm_node_id capacity free_flow_speed length num_lanes osm_road_category road_geometry speed_limit travel_time -i $inputGraph -co $inputDir/Graphs/${veh_name} -po $inputDir/Graphs/${psg_name}
